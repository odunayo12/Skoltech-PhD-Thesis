...
(other than $v$) adjacent to $v$. Given a Graph $\mathcal{G}$,
...
\begin{problem}
Given a Graph $G = (V, E)$, let $ \tilde{V} = V(G)\backslash V^*$. Denote by $\vartheta (G)$ the set of articulation points
associated with Graph $G$ defined by
\begin{equation}
...
each articulation point contained in $\vartheta (G)$. The traffic on $\vartheta (G_i)$ is represented by its volume
of edges $e \in E(G)$
incident on it and expressed as
\begin{equation}
  vol(\vartheta (G_i)) = \sum_{\vartheta (G_i)\in \mathcal{N}(G_i)} \delta(\vartheta (G_i))
\end{equation}
\end{problem}%
\begin{figure}[htb!]
  \centering
